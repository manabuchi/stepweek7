...
void *my_malloc(size_t size) {
  ...
  size_t min_diff = (size_t)-1; //最小の差を追跡する


  //Best fit フリーリスト全体を追跡して最適なフィットを見つける
  my_metadata_t *prev_current = NULL; // 要求サイズを満たすブロックが見つかった
  while (current) {
    if (current->size >= size) { //現在のベストフィットよりも小さい差(より良い解)が見つかった
      ...
      if (diff < min_diff) {
        ...
        best_fit_prev = prev_current;
        if (diff == 0) { // 完全に一致するサイズが見つかった場合、これ以上良いフィットはないのでループを抜ける。
          break;
        }
      }
      
    }
    prev_current = current; // 次のループのためにprev_currentを更新する
